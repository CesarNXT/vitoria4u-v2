...
import { ArrowUpDown, Pencil, Trash2, Bell, BellOff } from "lucide-react"
...
export const getColumns = ({ onEdit, onDelete }: ColumnsProps): ColumnDef<Profissional>[] => [
  ...
  {
    accessorKey: "notificarAgendamentos",
    header: "Notificações",
    cell: ({ row }) => {
      const notifyEnabled = row.original.notificarAgendamentos ?? true;
      return (
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const handleWhatsAppClick = () => {
        const phone = String(professional.phone);
      ...
      return (
        <div className="flex items-center justify-end gap-2">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="outline" size="icon" onClick={() => onEdit(professional)}>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="outline" size="icon" className="text-destructive hover:text-destructive hover:bg-destructive/10 border-destructive/50" onClick={() => onDelete(professional)}>
